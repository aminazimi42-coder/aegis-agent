...
- **Layer-3 installer folder** — `dist/AegisOperator-mac` is built by `scripts/build_mac_installer.sh`; it is unsigned, Terminal venv install is still required, and it is not a paid desktop shop. The uninstall script leaves `$HOME/.aegis` in place; the installer app is still unsigned.
...
- **Codesign with optional notary** — `scripts/codesign_operator.sh` signs only when a Developer ID Application identity is present in the keychain; without it the script prints `UNSIGNED` and exits 0. When `AEGIS_NOTARY_PROFILE` is set the script submits to notarytool with that keychain profile and prints the notarize-success token only after `stapler validate` succeeds; without a staple ticket it prints `NOTARY_SKIPPED` or `NOTARY_PENDING` and never claims a notarize success. The Mac folder stays unsigned when no identity exists and is not store-distributed software.
