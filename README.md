...
- **Developer ID signing wired** — `scripts/codesign_operator.sh` signs only when a Developer ID Application identity is present in the keychain; without the identity the script prints `UNSIGNED` and exits 0. After signing it runs `codesign --verify --deep --strict` and prints `SIGNED: <identity>` only if verify exits 0. The script does not invoke notarytool or stapler and prints `NOTARY_LOCKED_UNTIL_TICKET`. The Mac folder stays unsigned when no identity exists and is not store-distributed software.
