...
- **Codesign script, unsigned without identity** — `scripts/codesign_operator.sh` signs only when a Developer ID Application identity is present in the keychain; without it the script prints `UNSIGNED` and exits 0. The Mac folder stays unsigned and is not store-distributed software.
