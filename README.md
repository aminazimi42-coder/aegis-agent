...
- **Quit engine sends SIGTERM on 8741 only** — the operator `Quit engine` button POSTs `/api/v1/twin/quit`, which sends `SIGTERM` to the process bound on `127.0.0.1:8741`; a non-local caller is a typed deny, and `start_operator.sh` remains the sole start path.
